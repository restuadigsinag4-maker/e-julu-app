...
function App() {
  ...
  // Bab state (guru bisa tambah)
  const [babList, setBabList] = useState([
    { id:1, judul:'Bab 1 - Pengantar',    modul:'', modul2:'', video:'' },
    { id:2, judul:'Bab 2 - Konsep Dasar', modul:'', modul2:'', video:'' },
    { id:3, judul:'Bab 3 - Lanjutan',     modul:'', modul2:'', video:'' },
  ]);
  ...
  // Quiz soal (guru bisa tambah)
  const [quizSoalList, setQuizSoalList] = useState([
    { id:1, tipe:'pg',    soal:'Berapakah nilai dari 2 + 2?',         opsi:['A. 3','B. 4','C. 5','D. 6','E. 7'], kunci:'B' },
    { id:2, tipe:'pg',    soal:'Ibu kota Indonesia adalah?',          opsi:['A. Bandung','B. Surabaya','C. Jakarta','D. Medan','E. Makassar'], kunci:'C' },
    { id:3, tipe:'essay', soal:'Jelaskan pengertian bilangan prima!', opsi:[], kunci:'' },
  ]);
  const [soalBaru, setSoalBaru] = useState({ tipe:'pg', soal:'', opsi:['A. ','B. ','C. ','D. ','E. '], kunci:'A' });
  const [essayBaru, setEssayBaru] = useState('');

  // Hasil siswa (dummy)
  const [hasilSiswa] = useState([
    { nama:'Rista Br Sinaga', kelas:'10-A', durasi:'20 menit', modulDurasi:150, videoDurasi:12, pg:80, essay:'Bilangan prima adalah bilangan yang hanya bisa dibagi 1 dan dirinya sendiri.', nilaiEssay:null },
    { nama:'Budi Hutabarat',  kelas:'10-A', durasi:'15 menit', modulDurasi:30,  videoDurasi:5,  pg:60, essay:'Bilangan yang tidak bisa dibagi selain 1 dan dirinya.', nilaiEssay:null },
    { nama:'Sari Manalu',     kelas:'10-B', durasi:'5 menit',  modulDurasi:10,  videoDurasi:3,  pg:40, essay:'Bilangan prima contohnya 2, 3, 5, 7, 11.', nilaiEssay:85 },
  ]);
  ...
  // ── Deteksi keluar aplikasi saat quiz ──────────────────────────
  useEffect(() => {
  ...
  const mulaiTimerModul = () => {
    modulTimerRef.current = setInterval(() => {
      setModulDurasi(p => p+1);
    }, 60000);
  };
  ...
  const startKameraSiswa = async () => {
    try {
      ...
      videoRef.current.srcObject = stream; setCameraActive(true);
  ...
  const startKameraGuru = async () => {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ video:{ facingMode:'user' } });
      videoGuruRef.current.srcObject = stream; setCameraGuruActive(true);
    } catch { alert('Kamera tidak bisa diakses'); }
  };
  const ambilFotoGuru = () => {
  ...
  const validasiSiswa = () => {
    ...
    if(!fotoDiambil){ setSiswaError('Foto selfie wajib diambil!'); return; }
    setSiswaError('');
    setPage('menunggu');
  };
  const validasiGuru = () => {
    const f = guruForm;
    if(!f.nip||!f.nama||!f.namaPanggilan||!f.mapel||!f.jabatan||
    ...
    if(!fotoGuruDiambil){ setGuruError('Foto profil wajib diambil!'); return; }
    setGuruError('');
    setPage('menunggu');
  };
  ...
  const LogoEjulu = () => (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', marginBottom:'16px' }}>
      <img src="/bbb.png" alt="E-JULU" style={{ height:'64px' }} />
    </div>
  );
  const Footer = () => (
    <div style={{
  ...
  if(page==='menunggu') return (
    <div style={{ ...S.page, justifyContent:'center', textAlign:'center' }}>
      ...
      <div style={{ background:'rgba(255,255,255,0.07)', borderRadius:'12px', padding:'16px', marginBottom:'24px', width:'100%' }}>
        <p style={{ color:'#4fc3f7', fontSize:'13px', margin:0 }}>
          💡 Hubungi admin sekolah jika butuh bantuan atau konfirmasi lebih cepat.
        </p>
      </div>
    ...
    <div style={S.page}>
      ...
      <div style={{ width:'100%' }}>
        ...
        <div style={{ background:'rgba(255,193,7,0.1)', border:'1px solid #f0c040', borderRadius:'8px', padding:'10px 14px', marginBottom:'16px' }}>
          <p style={{ color:'#f0c040', fontSize:'12px', margin:0 }}>
            ⚠️ Akun harus sudah disetujui admin untuk bisa login.
          </p>
        </div>
        <button style={{ ...S.btnGold, borderRadius:'30px', justifyContent:'center', fontSize:'17px' }}
          onClick={()=>{ setUserRole('siswa'); setPage('dashboard'); }}>
          → Masuk
        </button>
      </div>
    </div>
  ...
  if(page==='loginGuru') return (
    <div style={S.page}>
      ...
      <div style={{ width:'100%' }}>
        ...
        <input style={S.input} type="text" placeholder="Masukkan NIP"
        ...
        <button style={{ ...S.btnGold, borderRadius:'30px', justifyContent:'center', fontSize:'17px' }}
          onClick={()=>{ setUserRole('guru'); setPage('dashboard'); }}>
          → Masuk
        </button>
      </div>
    </div>
  ...
  if(page==='dashboard') return (
    <div style={S.page}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', width:'100%', background:'rgba(255,255,255,0.07)', borderRadius:'12px', padding:'12px 20px', marginBottom:'24px' }}>
        ...
        <div style={{ color:'#f0e000', fontSize:'13px', fontWeight:'700' }}>
          {userRole==='guru'?'👨‍🏫 Mode Guru':'🎓 Mode Siswa'}
        </div>
        <div style={{ textAlign:'center', cursor:'pointer' }}>
          <div style={{ fontSize:'26px' }}>👤</div>
          <div style={{ fontSize:'11px', color:'#aaa' }}>Tentang</div>
        </div>
      </div>
      {[
      ...
      <button onClick={()=>{ setUserRole(null); setPage('role'); }}
  ...
  if(page==='forum') return (
    <div style={S.page}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', width:'100%' }}>
        {mapelList.map(m=>(
          <button key={m.id}
            onClick={()=>{ setSelectedMapel(m); setPage('forumBab'); }}
            style={{
              padding:'16px 10px', borderRadius:'14px', border:'none',
              background:`linear-gradient(135deg,${m.warna},${m.warna}99)`,
              color:'white', fontWeight:'bold', fontSize:'14px', cursor:'pointer',
              display:'flex', flexDirection:'column', alignItems:'center', gap:'8px',
              boxShadow:`0 4px 12px ${m.warna}44`,
            }}>
      ...
      <div style={{ width:'100%' }}>
        ...
        {userRole==='guru' && (
          <div style={{ ...S.card, border:'1px solid #27ae60', marginTop:'8px' }}>
            ...
            <input style={S.input} placeholder="Judul bab baru..." value={babBaru} onChange={e=>setBabBaru(e.target.value)} />
            <button onClick={()=>{
              if(!babBaru.trim()) return;
              setBabList(prev=>[...prev,{ id:Date.now(), judul:babBaru, modul:'', modul2:'', video:'' }]);
              setBabBaru('');
  ...
  if(page==='forumIsiBab') return (
    <div style={S.page}>
      ...
      <p style={{ color:'#aaa', fontSize:'13px', marginBottom:'24px' }}>{selectedMapel?.nama}</p>
      ...
      {userRole==='siswa' && (
        <div style={{ ...S.card, border:'1px solid #27ae60', marginBottom:'16px' }}>
          <p style={{ color:'#27ae60', fontWeight:'bold', fontSize:'13px', margin:0 }}>
            ...
            &nbsp;|&nbsp; Modul: {modulDurasi} mnt &nbsp;|&nbsp; Video: {videoDurasi} mnt
          </p>
        </div>
      )}
        ...
        <div style={{ ...S.card, border:'1px solid #2a5f7a' }}>
          ...
          {userRole==='siswa'
            ...
            : <div>
                ...
                <button onClick={()=>setBabList(prev=>prev.map(x=>x.id===selectedBab.id?{...x,modul:linkEdit.modul}:x))}
        ...
        <div style={{ ...S.card, border:'1px solid #2a5f7a' }}>
          ...
          {userRole==='siswa'
            ...
            : <div>
                ...
                <button onClick={()=>setBabList(prev=>prev.map(x=>x.id===selectedBab.id?{...x,modul2:linkEdit.modul2}:x))}
                  style={{ background:'#1e8449', border:'none', color:'white', borderRadius:'6px', padding:'8px 16px', cursor:'pointer', fontWeight:'bold', fontSize:'13px' }}>
                  💾 Simpan Link
                </button>
              </div>
        ...
        <div style={{ ...S.card, border:'1px solid #e74c3c' }}>
          ...
          {userRole==='siswa'
            ...
            : <div>
                ...
                <button onClick={()=>setBabList(prev=>prev.map(x=>x.id===selectedBab.id?{...x,video:linkEdit.video}:x))}
                  style={{ background:'#1e8449', border:'none', color:'white', borderRadius:'6px', padding:'8px 16px', cursor:'pointer', fontWeight:'bold', fontSize:'13px' }}>
                  💾 Simpan Link
                </button>
              </div>
        ...
        <div style={{ ...S.card, border:'1px solid #f39c12' }}>
          ...
          {userRole==='siswa'
            ? <button onClick={()=>{
                ...
                setTimer(20);
                setPage('quiz');
              }} style={{ ...S.btnOrange, marginTop:'0' }}>🚀 Mulai Quiz</button>
            : <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                ...
                <button onClick={()=>setPage('guruKelola')}
  ...
  if(page==='guruBuatQuiz') return (
    <div style={S.page}>
      ...
      {quizSoalList.map((q,i)=>(
        <div key={q.id} style={{ ...S.card, border:'1px solid #2a5f7a' }}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'8px' }}>
            ...
            <button onClick={()=>setQuizSoalList(prev=>prev.filter(x=>x.id!==q.id))}
          ...
          <textarea style={{ ...S.input, height:'70px', resize:'none' }}
            value={q.soal}
            onChange={e=>setQuizSoalList(prev=>prev.map(x=>x.id===q.id?{...x,soal:e.target.value}:x))} />
          {q.tipe==='pg' && (
            <div>
              <label style={S.label}>Pilihan Jawaban:</label>
              {q.opsi.map((op,j)=>(
                <input key={j} style={{ ...S.input, marginBottom:'6px' }} value={op}
                  onChange={e=>setQuizSoalList(prev=>prev.map(x=>x.id===q.id?{...x,opsi:x.opsi.map((o,k)=>k===j?e.target.value:o)}:x))} />
              ))}
              <label style={S.label}>Kunci Jawaban:</label>
              <select style={S.select} value={q.kunci}
                onChange={e=>setQuizSoalList(prev=>prev.map(x=>x.id===q.id?{...x,kunci:e.target.value}:x))}>
      ...
      <div style={{ ...S.card, border:'2px dashed #8e44ad', marginTop:'8px' }}>
        ...
        <label style={S.label}>Pilihan:</label>
        ...
        <button onClick={()=>{
          ...
          setQuizSoalList(prev=>[...prev,{ id:Date.now(), tipe:'pg', ...soalBaru }]);
      ...
      <div style={{ ...S.card, border:'2px dashed #f39c12', marginTop:'12px' }}>
        ...
        <button onClick={()=>{
          ...
          setQuizSoalList(prev=>[...prev,{ id:Date.now(), tipe:'essay', soal:essayBaru, opsi:[], kunci:'' }]);
          setEssayBaru('');
  ...
  if(page==='guruKelola') return (
    <div style={S.page}>
      ...
      <div style={{ display:'flex', gap:'8px', width:'100%', marginBottom:'16px' }}>
        {[
          { key:'pg',    label:'📝 Skor Pilber' },
          { key:'essay', label:'✍️ Jawaban Essay' },
          { key:'modul', label:'📖 Aktivitas Modul' },
        ].map(t=>(
      ...
      {lihatTab==='pg' && hasilSiswa.map((s,i)=>(
        <div key={i} style={{ ...S.card, border:'1px solid #2a5f7a' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <div>
              <p style={{ fontWeight:'bold', margin:'0 0 2px', fontSize:'15px' }}>{s.nama}</p>
              <p style={{ color:'#aaa', fontSize:'12px', margin:0 }}>Kelas {s.kelas}</p>
            </div>
            <div style={{ textAlign:'right' }}>
              <p style={{ color:'#2ecc71', fontWeight:'bold', margin:0 }}>{s.pg}/100</p>
              <p style={{ color:'#aaa', fontSize:'11px', margin:0 }}>{s.pg>=60?'✅ Lulus':'❌ Remedial'}</p>
            </div>
          </div>
          <div style={{ height:'6px', background:'rgba(255,255,255,0.1)', borderRadius:'3px', marginTop:'10px' }}>
            <div style={{ height:'6px', background: s.pg>=60?'#2ecc71':'#e74c3c', borderRadius:'3px', width:`${s.pg}%` }} />
          </div>
        </div>
      ))}
      ...
      {lihatTab==='essay' && hasilSiswa.map((s,i)=>(
        <div key={i} style={{ ...S.card, border:'1px solid #f39c12' }}>
          <p style={{ fontWeight:'bold', margin:'0 0 4px', fontSize:'15px' }}>{s.nama}</p>
          <p style={{ color:'#aaa', fontSize:'12px', margin:'0 0 10px' }}>Kelas {s.kelas}</p>
          <div style={{ background:'rgba(255,255,255,0.05)', borderRadius:'8px', padding:'10px', marginBottom:'10px' }}>
            <p style={{ color:'#ccc', fontSize:'13px', margin:0, lineHeight:'1.6' }}>{s.essay}</p>
          </div>
          {s.nilaiEssay!==null
            ? <p style={{ color:'#2ecc71', fontWeight:'bold', fontSize:'14px' }}>✅ Nilai: {s.nilaiEssay}/100</p>
            : <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
                ...
                <button style={{ background:'#1e8449', border:'none', color:'white', borderRadius:'8px', padding:'10px 14px', cursor:'pointer', fontWeight:'bold', whiteSpace:'nowrap' }}>
                  💾 Simpan
                </button>
              </div>
          }
      ...
      <div style={{ width:'100%' }}>
        ...
        <div style={{ display:'flex', gap:'10px' }}>
          <div style={{ flex:1 }}>
            <label style={S.label}>Password *</label>
            <div style={S.pwWrap}>
              <input style={{ ...S.input, paddingRight:'36px' }} type={showPassword?'text':'password'} placeholder="••••••••"
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={S.pwWrap}>
              <input style={{ ...S.input, paddingRight:'36px' }} type={showConfirmPassword?'text':'password'} placeholder="••••••••"
        ...
        <label style={S.label}>FOTO SELFIE (Wajib) *</label>
        <div style={{ background:'rgba(255,255,255,0.05)', borderRadius:'12px', padding:'16px', marginBottom:'14px', textAlign:'center', border:'1px solid #2a5f7a' }}>
          {!cameraActive&&!fotoDiambil&&(<div>
            <div style={{ fontSize:'40px', marginBottom:'6px' }}>📷</div>
            <p style={{ color:'#aaa', fontSize:'13px', margin:'0 0 10px' }}>Tombol akan mengaktifkan kamera depan</p>
            <button onClick={startKameraSiswa} style={{ background:'#1565c0', color:'white', border:'none', borderRadius:'8px', padding:'10px 20px', cursor:'pointer', fontWeight:'bold', fontSize:'14px' }}>📸 AKTIFKAN KAMERA</button>
          </div>)}
          {cameraActive&&(<div>
            <video ref={videoRef} autoPlay playsInline style={{ width:'100%', borderRadius:'8px', marginBottom:'10px' }} />
            <button onClick={ambilFotoSiswa} style={{ background:'#27ae60', color:'white', border:'none', borderRadius:'8px', padding:'10px 20px', cursor:'pointer', fontWeight:'bold', fontSize:'14px' }}>📸 AMBIL FOTO</button>
          </div>)}
          ...
          {fotoDiambil&&(<div>
            ...
            <button onClick={()=>{ setFotoDiambil(false); startKameraSiswa(); }} style={{ background:'#e07b00', color:'white', border:'none', borderRadius:'8px', padding:'8px 16px', cursor:'pointer', fontSize:'13px' }}>🔄 Ambil Ulang</button>
          </div>)}
        </div>
        <button style={S.btnOrange} onClick={validasiSiswa}>DAFTAR SEKARANG</button>
      </div>
    </div>
  );
  ...
  if(page==='registerGuru') return (
    <div style={S.page}>
      ...
      <div style={{ width:'100%' }}>
        <label style={S.label}>NIP *</label>
        <input style={S.input} placeholder="Nomor Induk Pegawai" value={guruForm.nip} onChange={e=>updateGuruForm('nip',e.target.value)} />
        <label style={S.label}>Nama Lengkap *</label>
        ...
        <label style={S.label}>FOTO PROFIL (Wajib) *</label>
        <div style={{ background:'rgba(255,255,255,0.05)', borderRadius:'12px', padding:'16px', marginBottom:'14px', textAlign:'center', border:'1px solid #2a5f7a' }}>
          {!cameraGuruActive&&!fotoGuruDiambil&&(<div>
            <div style={{ fontSize:'40px', marginBottom:'6px' }}>📷</div>
            <p style={{ color:'#aaa', fontSize:'13px', margin:'0 0 10px' }}>Tombol akan mengaktifkan kamera depan</p>
            <button onClick={startKameraGuru} style={{ background:'#1565c0', color:'white', border:'none', borderRadius:'8px', padding:'10px 20px', cursor:'pointer', fontWeight:'bold', fontSize:'14px' }}>📸 AKTIFKAN KAMERA</button>
          </div>)}
          {cameraGuruActive&&(<div>
            <video ref={videoGuruRef} autoPlay playsInline style={{ width:'100%', borderRadius:'8px', marginBottom:'10px' }} />
            <button onClick={ambilFotoGuru} style={{ background:'#27ae60', color:'white', border:'none', borderRadius:'8px', padding:'10px 20px', cursor:'pointer', fontWeight:'bold', fontSize:'14px' }}>📸 AMBIL FOTO</button>
          </div>)}
          <canvas ref={canvasGuruRef} style={{ width:'100%', borderRadius:'8px', display:fotoGuruDiambil?'block':'none', marginBottom:'10px' }} />
          {fotoGuruDiambil&&(<div>
            <p style={{ color:'#2ecc71', fontWeight:'bold', fontSize:'14px', margin:'0 0 8px' }}>✅ Foto berhasil!</p>
            <button onClick={()=>{ setFotoGuruDiambil(false); startKameraGuru(); }} style={{ background:'#e07b00', color:'white', border:'none', borderRadius:'8px', padding:'8px 16px', cursor:'pointer', fontSize:'13px' }}>🔄 Ambil Ulang</button>
          </div>)}
        </div>
        <button style={S.btnOrange} onClick={validasiGuru}>DAFTAR SEKARANG</button>
      </div>
    </div>
  );
}
